...
json  = require 'json' --added to read json documents
...
function setup()
  ...
  -- reading the file and decoding it to json
  local pollution_json = read_file("all_data_pollution.json")
  if pollution_json == nil
    then
      print("all_data_pollution.json")
  end
  local pollution_data = json.decode(pollution_json)["data"]
  segments = {}
  -- adding each segmentId to the table above

  for k, v in pairs(pollution_data) do
    table.insert(segments, {segmentId = v["segmentId"], avg = v["avg"], max = v["max"], distance = v["distance"]})
  end

  local bike_net_json = read_file("leipzig_bikenet.json")
  if bike_net_json == nil
  then
    print("leipzig_bikenet.json")
  end

  local bike_net = json.decode(bike_net_json)
  roads_leipzig = {}

  for k, v in pairs(bike_net["data"]) do
    table.insert(roads_leipzig, {wayid = v["way_id"], distance = v["length"]})
  end
...
function process_way(profile, way, result)
  ...
  --override rates
  if (result.forward_mode ~= mode.inaccessible and result.forward_speed > 0)
  then
    for k, v in pairs(segments) do
      if v.segmentId == way:id() 
      then
        if v.avg == 0.0
        then
          result.forward_rate = 1.0 / (0.1 * v.distance) * 1000
          result.backward_rate = 1.0 / (0.1 * v.distance) * 1000
        else
          result.forward_rate = 1.0 / (v.avg * v.distance) * 1000
          result.backward_rate = 1.0 / (v.avg * v.distance) * 1000
        adjusted = true
        end
      end 
    end
    ...
    if adjusted == false
    then
      --local avg_all = 27.642123
      local avg_all = 1000.0
      for k, v in pairs(roads_leipzig) do
        if v.wayid == way:id()
        then
          result.forward_rate = 1.0 / (avg_all * v.distance) * 1000
          result.backward_rate = 1.0 / (avg_all * v.distance) * 1000
        end
      end
    end
  end
